...
Camera::Camera(const Scene &scene, const Vector3 &location, const Vector3 &focus, const Vector3 &up,
    double fustrumLen, double focalLen, double blurRadius, double topWidth,
    int resX, int resY, int longevity) :
    location(location), rotation((focus - location).normalize()),
    up(rotation.crossProduct(up).crossProduct(rotation).normalize()),
    environment(scene), fustrumLen(fustrumLen), focalLen(focalLen),
    topWidth(topWidth), blurRadius(blurRadius), resX(resX), resY(resY), longevity(longevity), image(new Colour[resX * resY])
{
